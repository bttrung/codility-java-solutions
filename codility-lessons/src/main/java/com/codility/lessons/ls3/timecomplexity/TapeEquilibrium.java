...
//        int a[] = {1, 1};
    ...
    public int solution(int[] a) {
        ...
        BigInteger minDiff = BigInteger.valueOf(Integer.MAX_VALUE);
        BigInteger left = BigInteger.ZERO;
        for (int i = 1; i < a.length; i++) {
            left = left.add(BigInteger.valueOf(a[i - 1]));

            BigInteger right = sum.subtract(left);

            BigInteger distance = left.subtract(right).abs();

            if (distance.compareTo(minDiff) < 0) {
                minDiff = distance;
            }
        }
